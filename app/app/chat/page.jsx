...
export default function ChatIndex() {
  ...
  myCommunities.forEach(c => {
    const communityChannels = channels.filter(ch => ch.community_id === c.id || ch.communityId === c.id);
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '24px', paddingBottom: '100px' }}>
        ...
        <div style={{ color: 'var(--slate-400)', textAlign: 'center', padding: '60px 0' }}>Loading chats...</div>
      </div>
    );
  }
  ...
  return (
    <div className="view-chat" style={{ paddingBottom: '100px', display: 'flex', flexDirection: 'column', height: '100vh' }}>
            ...
            {inboxItems.map((item, i) => {
              ...
              return (
                <div 
                  ...
                  onClick={() => router.push(item.type === 'dm' ? `/chat/dm/${item.id}` : `/chat/${item.comm.id}/${item.id}`)}
      ...
      <div style={{ position: 'fixed', bottom: '90px', right: '24px', zIndex: 50 }}>
        <button 
      ...
      {/* New Chat Modal */}
      {showCreateModal && (
        <div className="modal-overlay" style={{ display: 'flex', flexDirection: 'column', padding: '20px', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(4px)' }}>
          <div className="modal-content" style={{ flex: 1, display: 'flex', flexDirection: 'column', maxHeight: '80vh', overflow: 'hidden', background: 'var(--slate-900)', borderRadius: '24px', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ fontFamily: 'var(--font-heading)', margin: 0, color: 'white' }}>
